...
const ShowComment: React.FC<ShowCommentProps> = ({
  ...
  const deleteComment = async (commentId: string) => {
    try {
      const res = await fetch(`${API}api/comment/delete/${commentId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const renderReplies = (parentId: string) => {
    return comments
      .filter((reply) => reply.parent_comment_id?.toString() === parentId)
      .map((reply) => (
        <div
          key={reply._id}
          className="reply bg-gray-100 shadow-md rounded-lg p-4 mt-4"
        >
          ...
          <p className="reply-body text-gray-700">{reply.description}</p>
        </div>
      ));
  };
            ...
            >
              ...
              <div className="flex gap-4 sm:gap-5 p-1 mt-2">
                ...
                <p
                  ...
                  onClick={() =>
                    setShowReplies(
                      showReplies === comment._id ? null : comment._id
                    )
                  }
                >
