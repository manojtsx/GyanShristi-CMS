...
const SideMenuBarEditor = () => {
  ...
  const menuItems = [
    { icon: <GoHome />, label: "Home", path: "/editor/dashboard" },
    {
    ...
    },
    {
      ...
      label: "User",
      submenu: [
        { label: "Viewer", path: "/editor/viewer" },
        { label: "Author", path: "/editor/author" },
      ],
    },
  ];
  ...
  return (
    <div
      className={`h-screen bg-[#011936] ${open ? "w-56" : "w-20"
        } duration-300 fixed left-0 top-0 z-10 flex flex-col`}
    >
      <BsArrowLeftShort
      ...
      <ul className=" mt-10 flex-grow">
        {menuItems.map((item, index) => (
          <li key={index}>
            <div
              className={`flex items-center p-2 text-gray-300 mb-2 rounded-md cursor-pointer  ${activeMenuItem === item.label
                  ? "bg-gray-700"
                  : "hover:bg-gray-700"
            ...
            {item.submenu && userMenuOpen && open && (
              <ul>
                {item.submenu.map((subItem, subIndex) => (
                  <li
                    key={subIndex}
                    className={`flex items-center p-1 ml-8 text-[#D9D9D9] rounded-md gap-x-4 cursor-pointer text-sm ${activeMenuItem === subItem.label
                        ? "bg-gray-700"
                        : "hover:bg-gray-700"
                      }`}
                    onClick={() =>
